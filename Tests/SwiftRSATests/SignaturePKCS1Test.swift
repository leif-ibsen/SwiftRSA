...
class SignaturePKCS1Test: XCTestCase {
    ...
    struct testStruct {
        ...
        init(msg: String, sig: String, kind: MessageDigest.Kind, ok: Bool = true) {
            self.msg = msg
            self.sig = sig
            self.kind = kind
            self.ok = ok
...
"""
    ...
    func test1024() throws {
        ...
        for t in tests1024 {
            ...
            XCTAssertEqual(t.ok, pubKey.verifyPKCS1(signature: sig, message: msg, kind: t.kind))
            if t.ok {
                let sig1 = try privKey.signPKCS1(message: msg, kind: t.kind)
