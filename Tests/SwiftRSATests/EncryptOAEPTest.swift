...
class EncryptOAEPTest: XCTestCase {
    ...
    struct testStruct {
        ...
        init(msg: String, cipher2048: String, cipher3072: String, cipher4096: String, kind: MessageDigest.Kind, label: String = "") {
            self.msg = msg
...
"""
    ...
    func test1() throws {
        ...
        for t in tests {
            ...
            let msg2048 = try privKey2048.decryptOAEP(cipher: cipher2048, kind: t.kind, label: label)
            XCTAssertEqual(msg2048, Utilities.hex2bytes(t.msg))
            let cipher1 = try pubKey2048.encryptOAEP(message: Utilities.hex2bytes(t.msg), kind: t.kind, label: label)
            let msg1 = try privKey2048.decryptOAEP(cipher: cipher1, kind: t.kind, label: label)
            XCTAssertEqual(msg1, msg2048)

            let cipher3072 = Utilities.hex2bytes(t.cipher3072)
            let msg3072 = try privKey3072.decryptOAEP(cipher: cipher3072, kind: t.kind, label: label)
            XCTAssertEqual(msg2048, Utilities.hex2bytes(t.msg))
            let cipher2 = try pubKey3072.encryptOAEP(message: Utilities.hex2bytes(t.msg), kind: t.kind, label: label)
            let msg2 = try privKey3072.decryptOAEP(cipher: cipher2, kind: t.kind, label: label)
            XCTAssertEqual(msg2, msg3072)

            let cipher4096 = Utilities.hex2bytes(t.cipher4096)
            let msg4096 = try privKey4096.decryptOAEP(cipher: cipher4096, kind: t.kind, label: label)
            XCTAssertEqual(msg4096, Utilities.hex2bytes(t.msg))
            let cipher3 = try pubKey4096.encryptOAEP(message: Utilities.hex2bytes(t.msg), kind: t.kind, label: label)
            let msg3 = try privKey4096.decryptOAEP(cipher: cipher3, kind: t.kind, label: label)
